...
- `docs/04-Concepts/architecture/FROZEN-BACKLOG.md` (30 consolidated deferrals)
...
| D07 | rules/ROADMAP.md §1.7 | parked_task | 4 | HIGH | **PARTIAL 2026-04-20**: Phase B-alpha (Jaccard) MVP shipped — `lib/reinvention_semantic.py` + `hooks/reinvention-check.sh` integration behind `REINVENTION_PHASE_B=1` gate. Design ADR-029b accepted. 5/5 tests pass, hook p95 160 ms (< 300 ms SLO). Phase B-beta (embeddings) and Phase B-γ (hard-block) still deferred pending 7-day dogfood + precision measurement per ADR-029b §8. | Dogfood Phase B-alpha, measure precision at threshold 0.3, then draft ADR-029c for embeddings | 1 |
