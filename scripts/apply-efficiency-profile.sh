...
run_claude_code_driver() {
  ...
  for hook in self-install.sh session-init.sh infra-health.sh subagent-context-injector.sh \
    ...
    content-policy.sh doc-sync-detector.sh claim-validator.sh post-agent-verify.sh orchestrator-claim-gate.sh pre-commit-content-hash-dedupe.sh concurrent-write-guard.sh plan-claim-validator.sh completion-gate.sh \
...
echo "  PreToolUse Bash: rate-limit-precheck.sh, agent-bash-cwd-enforcer.sh, rate-limiter.sh, destructive-rm-blocker.sh, destructive-git-blocker.sh, symlink-mutation-guard.sh, scope-marker-portability-gate.sh, git-commit-scope-guard.sh, orchestrator-claim-gate.sh, pre-commit-content-hash-dedupe.sh"
