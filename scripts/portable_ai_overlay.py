...
def _profile_rows(root: Path, primitive_rows: list[tuple[str, dict[str, Any]]]) -> list[tuple[str, dict[str, Any]]]:
    ...
    for harness in all_harnesses:
        ...
        profile = {
            ...
            "compiler_gap_policy": "Profiles expose declared fidelity; native IDE files must be emitted by governed projection drivers or a future adapter compiler that preserves fidelity.",
            "harness": harness,
...
def build_overlay(root: Path) -> dict[str, str]:
    ...
    context = {
        ...
        "consumer_package_policy": "Consumer-friendly .ai packages and native IDE files must be generated by governed projection drivers or a future adapter compiler from canonical contracts, preserving projection fidelity.",
        "native_file_emission": False,
