...
# ---------------------------------------------------------------------------
# Integration test with real Paperclip repo (optional)
# ---------------------------------------------------------------------------
...
class TestPaperclipIntegration:
    ...
    def test_paperclip_config_schemas_found(self) -> None:
        """Paperclip repo has detectable config schemas."""
        re_eng = ReverseEngineer()
        schemas = re_eng.analyze_config_schema("/tmp/paperclip-build")
        assert len(schemas) > 0, "Expected at least one config schema in Paperclip"

    def test_paperclip_env_vars_found(self) -> None:
        """Paperclip repo has detectable environment variables."""
        re_eng = ReverseEngineer()
        env_vars = re_eng.analyze_env_vars("/tmp/paperclip-build")
        assert len(env_vars) > 0, "Expected at least one env var in Paperclip"

    def test_paperclip_docker_setup_found(self) -> None:
        """Paperclip repo has Docker configuration."""
        re_eng = ReverseEngineer()
        setup = re_eng.analyze_docker_setup("/tmp/paperclip-build")
        assert setup.base_image is not None or len(setup.compose_services) > 0
