...
| `scripts/cos-manifest-tier-claim-audit --json` | Classifies primitive lifecycle distribution claims that still depend on maintainer knowledge or look like lab/demotion candidates. |
...
scripts/cos-manifest-tier-claim-audit --json         # warn while manifest distribution claims still expose maintainer-knowledge/demotion candidates
