...
|---|---|---|---|---|---|
| 1 | [Depyler](https://github.com/paiml/depyler) | Py→Rust | Beta v4.1, MCP | MIT | **Top untested alt** — single-shot compile claim; add to lane. |
...
1. **Add Depyler as a third transpiler in the existing lane**. It matches the lane's contract (annotated Py → Rust), claims single-shot compile, and ships under MIT — minimal risk, maximum signal. Run capability-mode + script-mode in the same harness.
