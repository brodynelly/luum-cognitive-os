<!--
RECONCILIATION STATUS: TOMBSTONE (strengthened) — 2026-05-11 (Opus refinement of 2026-05-10 Sonnet pass)
Reconciled-by: P3 plan triage Opus re-triage (see docs/06-Daily/reports/p3-plan-triage-2026-05-10.md §Opus refinement)
Decision: TOMBSTONE — confirmed. ADR-tombstone slot recommended.
Opus rationale (refinement of Sonnet TOMBSTONE-by-doctrine): Sonnet justified TOMBSTONE via the doctrine's "Distributed workflow engines: DEFER" clause. That clause targets Temporal/NATS/Firecracker/OPA — heavy *distributed* engines. The local YAML+networkx+asyncio engine this plan proposes is not quite that category and the doctrine alone does not strictly forbid it. The stronger reason to TOMBSTONE is **coexistence-with-shipped-substrate**: `.cognitive-os/workflows/` already contains feature-pipeline.yaml + bugfix-pipeline.yaml, and `docs/08-References/root/adw-patterns.md` documents the ADW model with 5 named pipelines (feature, bugfix, refactor, sre, review) — i.e., COS already has a lightweight declarative workflow capability. ADR-036 sprint orchestration primitives, `@event_wrap`, and ADR-226 cover the remaining slices. Implementing lib/workflow_engine.py + lib/workflow_types.py + WorkflowEngine/WorkflowParser/DAGBuilder/StateManager would directly duplicate this without governance value. This is TOMBSTONE-by-coexistence (stronger than Sonnet's TOMBSTONE-by-doctrine) plus the doctrine reinforces the result. RECOMMEND ADR-tombstone slot to formally close the workflow-engine concept and reference adw-patterns.md + .cognitive-os/workflows/ as the canonical lightweight workflow substrate. If a Shape-B trigger ever fires for federation/cluster runtime per ADR-132, revisit via ADR-254 manifest/audit/research-check path — not by reviving this plan as-is.
Sonnet 2026-05-10 decision: TOMBSTONE. Opus agreement: TOMBSTONE confirmed; rationale strengthened from doctrine-only to coexistence-with-shipped-ADW-substrate. Recommend formal ADR-tombstone.
...
docs/07-Capabilities/skills-catalog.md       — add workflow-engine entry
```
