...
Monitors API token consumption per session and warns/blocks before exhausting
the hourly budget. Thresholds: 50% INFO, 80% WARN, 95% BLOCK.

Renamed from rate_limit_protection (which was misleading — this monitors token
*consumption*, not action-count rate limits).  For action-count governing
(tool calls/minute) see lib/rate_limiter.py.
