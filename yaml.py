...
Delegates to installed PyYAML when available. When unavailable, provides the
safe_load/safe_dump subset needed by repository CLIs that run under system
python3 in behavior tests and consumer projections.
"""
...
_EXCLUDE = {_THIS_DIR, _THIS_DIR / "scripts"}
_search_paths = [item for item in sys.path if item and Path(item).resolve() not in _EXCLUDE]
_spec = importlib.machinery.PathFinder.find_spec("yaml", _search_paths)
_COS_YAML_DELEGATED = False
if _spec and _spec.origin and Path(_spec.origin).resolve() != _THIS:
    try:
        ...
        _COS_YAML_DELEGATED = True
    except Exception:
        sys.modules[__name__] = sys.modules.get(__name__, None) or sys.modules[__name__]

if not _COS_YAML_DELEGATED:
    ...
    def _preprocess(text: str) -> list[tuple[int, str]]:
        ...
        while i < len(raw_lines):
            ...
            if stripped.endswith(": >") or stripped.endswith(": |") or stripped.endswith(": >-") or stripped.endswith(": |-"):
                ...
                continue
            if ":" not in stripped and not stripped.startswith("- ") and out:
                prev_indent, prev_text = out[-1]
                out[-1] = (prev_indent, f"{prev_text} {stripped}")
            else:
