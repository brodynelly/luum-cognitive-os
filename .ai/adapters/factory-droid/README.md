# Factory Droid portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `factory-droid`
- status: `implemented`
- proof level: `structural`
- projection mode: `agents-md-factory-skills-mcp-hooks`

## Settings paths

- `AGENTS.md`
- `.factory/mcp.json`
- `.factory/settings.json`
- `.factory/skills/cognitive-os/SKILL.md`

## Rule

Read `.ai/profiles/factory-droid.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
