...
| `hooks/context-watchdog.sh` | ON_DEMAND | fire_count_7d=0, registered=True, has_test=True | registered + covered by test — legit sleeper (fires when triggered) |
...
| `hooks/reinvention-check.sh` | REAL | fire_count_7d=8, registered=True | fires actively (8 rows in hook-health.jsonl last 7d) |
...
| `hooks/session-learning.sh` | REAL | fire_count_7d=1, registered=True | fires actively (1 rows in hook-health.jsonl last 7d) |
...
| `lib/engram_claims.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=8059 | covered by test — legit sleeper (imported by test only) |
| `lib/engram_client.py` | REAL | callers=6, size_bytes=6605 | imported by 6 non-test caller(s) |
...
| `lib/engram_locks.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=9466 | covered by test — legit sleeper (imported by test only) |
...
| `lib/session_bus.py` | REAL | callers=2, size_bytes=2862 | imported by 2 non-test caller(s) |
...
| `lib/work_identity.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=7855 | covered by test — legit sleeper (imported by test only) |
...
| `scripts/apply-efficiency-profile.sh` | ON_DEMAND | callers=0, has_test=True, size_bytes=11798 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/cos-claims.sh` | DORMANT | callers=0, size_bytes=5174 | no observable production use, no test, no on-demand marker |
...
| `scripts/cos-coordination-status.sh` | ON_DEMAND | callers=0, has_test=True, size_bytes=296 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/cos-fingerprint.sh` | DORMANT | callers=0, size_bytes=4332 | no observable production use, no test, no on-demand marker |
...
| `scripts/cos-locks.sh` | DORMANT | callers=0, size_bytes=4917 | no observable production use, no test, no on-demand marker |
...
| `scripts/cos_work_inventory.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=39597 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/derived_artifact_gate.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=5725 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/merge-to-main.sh` | ON_DEMAND | callers=0, has_test=True, size_bytes=2383 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/orchestrator_claim_gate.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=9768 | covered by test — legit sleeper (test proves it works when called) |
...
| `scripts/primitive_gap_snapshot.py` | ON_DEMAND | callers=0, has_test=True, size_bytes=19164 | covered by test — legit sleeper (test proves it works when called) |
