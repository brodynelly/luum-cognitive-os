# DeepSeek provider integrations portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `deepseek-provider`
- status: `planned`
- proof level: `none`
- projection mode: `provider-via-existing-harness`

## Settings paths

- none declared

## Rule

Read `.ai/profiles/deepseek-provider.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
