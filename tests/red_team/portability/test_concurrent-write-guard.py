...
Verifies the concurrent-write-guard Codex projection strategy:
  - Claude Code: PreToolUse[Edit|Write] — per-file locking (native projection).
  - Codex: UserPromptSubmit (prompt) via hooks/concurrent-write-guard-codex-proxy.sh
    — prompt-time stale-lock scan (degraded-but-honest projection, warning only).

The portability invariant tested here is that the proxy fires under Codex
harness simulation and surfaces cross-session lock contention before edits
begin. The mutual-exclusion invariant (per-file blocking) is harness-advantaged
for Claude Code and is documented as such in ADR-111 §Gate-3.
