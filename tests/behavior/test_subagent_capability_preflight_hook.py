...
def test_subagent_capability_hook_blocks_explore_artifact_launch(project_root):
    payload = '{"tool_name":"Agent","tool_input":{"subagent_type":"Explore","prompt":"Research and write docs/reports/output.md"}}'
...
def test_subagent_capability_hook_allows_parent_persistence(project_root):
    payload = '{"tool_name":"Agent","tool_input":{"subagent_type":"Explore","prompt":"Research read-only and return result only; parent will persist docs/reports/output.md"}}'
    result = subprocess.run(
        ["bash", str(project_root / "hooks" / "subagent-capability-preflight.sh")],
        input=payload,
        capture_output=True,
