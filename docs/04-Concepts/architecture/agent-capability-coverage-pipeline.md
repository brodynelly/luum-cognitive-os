...
This is intentionally narrow. Claude Code and OpenAI Codex currently prove native/settings lifecycle projection. Cursor, VS Code Copilot, Qwen Code, and Kimi Code prove structural project-local instruction/config/context projection only. OpenCode currently proves structural `opencode.json` projection only, but its official permission and plugin lifecycle surfaces are runtime-capable and should be used by a future COS OpenCode plugin adapter instead of a parallel enforcement layer. Shell/CI proves structural command/workflow projection only. Devin, Google Antigravity, MiniMax MaxClaw, and DeepSeek provider integrations remain planned until they have their own projection proof.
...
`manifests/harness-projection.yaml` is the authoritative list of IDEs/harnesses considered by ACC. Claude Code and OpenAI Codex are `implemented` native/settings harnesses. Cursor, VS Code Copilot, Qwen Code, and Kimi Code are `implemented` structural instruction/config/context harnesses. OpenCode is `implemented` for structural config projection and separately marked runtime-plugin-capable through permissions and plugin lifecycle events; ACC must not count that as runtime enforcement until the COS adapter smoke exists. Shell/CI is an `implemented` structural command/workflow harness. Devin, Google Antigravity, MiniMax MaxClaw, and DeepSeek provider integrations are declared as `planned`. Planned harnesses are roadmap scope only: they are reported as unverified and never inherit Claude/Codex projection proof.
