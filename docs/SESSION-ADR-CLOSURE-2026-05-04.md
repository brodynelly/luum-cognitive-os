...
| [ADR-042-valkey-local-daemon](adrs/ADR-042-valkey-local-daemon.md) | `pending_evidence` | `accepted` | `evidence-only` | ADR-042: Valkey Local Daemon — Extract from Docker (D34 Partial) — Evidence references exist, but the ADR does not state completion clearly **Batch-2 classification:** `scripts/cos-valkey-local.sh`, `hooks/valkey-ensure.sh`, `lib/agent_bus.py` local fallback, and `tests/integration/test_valkey_local_daemon.py` exist and passed in this session. Remaining Paperclip/PostgreSQL Docker items are explicitly deferred outside Valkey scope. | **Final classification:** Local daemon scripts exist; close by evidence/status update and keep optional-service posture.
