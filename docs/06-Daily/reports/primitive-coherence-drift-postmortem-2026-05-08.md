...
| Reports with sensitive paths or patterns | 🟢 Mitigated by pre-public/report-publish gates | `hook-fast` runs before writes to `docs/06-Daily/reports/`, `docs/01-Build-Log/history/`, and `docs/08-References/business/`; `cos-pre-public-risk-audit` currently passes. |
