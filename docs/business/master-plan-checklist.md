...
- [x] Codex has a governed fallback layer for non-native Agent and Edit/Write hook chains through `scripts/cos-codex-guard.py`, so portability gaps are explicit rather than silently accepted.
