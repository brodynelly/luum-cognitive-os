# Qoder CLI portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `qoder`
- status: `implemented`
- proof level: `structural`
- projection mode: `agents-md-project-mcp-settings`

## Settings paths

- `AGENTS.md`
- `.mcp.json`
- `.qoder/settings.json`

## Rule

Read `.ai/profiles/qoder.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
