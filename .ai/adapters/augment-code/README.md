# Augment / Auggie portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `augment-code`
- status: `implemented`
- proof level: `structural`
- projection mode: `augment-cli-flags-rules-mcp`

## Settings paths

- `.augment/rules/cognitive-os.md`
- `.augment/mcp.json`
- `.augment/README.md`

## Rule

Read `.ai/profiles/augment-code.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
