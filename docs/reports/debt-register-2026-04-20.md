...
- `.cognitive-os/plans/roadmaps/stabilization-roadmap.md` (P2/P3 residue)
...
| D07 | rules/ROADMAP.md §1.7 | parked_task | 4 | HIGH | **PARTIAL 2026-04-20**: Phase B-α (Jaccard) MVP shipped — `lib/reinvention_semantic.py` + `hooks/reinvention-check.sh` integration behind `REINVENTION_PHASE_B=1` gate. Design ADR-029b accepted. 5/5 tests pass, hook p95 160 ms (< 300 ms SLO). Phase B-β (embeddings) and Phase B-γ (hard-block) still deferred pending 7-day dogfood + precision measurement per ADR-029b §8. | Dogfood Phase B-α, measure precision at threshold 0.3, then draft ADR-029c for embeddings | 1 |
...
| D22 | ADR-028 OQ#8 | deferred_adr | 4 | MEDIUM | Orchestrator itself has no heartbeat — sub-agents do. Silent orchestrator hangs undetectable by so-vitals. | Decide: add in Phase D or accept | 0.25 | **STALE**: `agent_bus.OrchestratorSubscriber` already tracks orchestrator heartbeats every 5s — see ADR-028b. Debt item obsolete. |
...
| D34 | FROZEN-BACKLOG #21 | uncommitted_note | >30 | ~~MEDIUM~~ **RESOLVED 2026-04-30** | Docker→pip phase 3 — **Valkey RESOLVED** (ADR-042); **Paperclip DEFERRED** (ADR-043 — no standalone binary; Docker demoted to `profiles: [legacy]`; `scripts/cos-paperclip-local.sh` stub ready to activate); **PostgreSQL RESOLVED** (ADR-045, `scripts/cos-postgres-local.sh`, port 5433, `lib/postgres_client.py`). All three services addressed; Docker compose profiles set to `legacy`. D34 CLOSED. | — | — |
...
| D43 | FROZEN-BACKLOG §P1 Core vs Extensions | uncommitted_note | >14 | **PARTIAL (2026-04-20)** | Core-vs-extensions audit complete (`docs/architecture/core-vs-extensions-audit-2026-04-20.md` — 581 components classified, 22% CORE). `packages/cos-advisory-llm/` extracted as Wave 1 POC: 3 LLM hooks moved, backwards-compat symlinks in place, both profiles verified green. 14 packs + shim cleanup remain per migration plan (21 waves → v1.0). **Cross-ref**: ADR-033 (harness-agnostic event capture) is the same decoupling philosophy applied to the capture layer — `lib/harness_adapter/` extracts harness-specific logic from the hook chain, making it portable to OpenCode/Aider/Cursor/Continue. | Execute waves 2–21 per `.cognitive-os/plans/architecture/core-vs-extensions-migration-plan.md` | 10 |
...
| D45 | orchestrator-2026-04-20 | production_bug | 0 | **RESOLVED fully** | Rate-limiter compounding retry loop — all 3 remaining gaps closed: (gap A) drainer-as-executor: `hooks/rate-limit-drain.sh` subprocess.run() safe queued bash_command items with `safe_to_execute()` allowlist + audit to `rate-limit-executed.jsonl` + re-enqueue if still blocked; (gap B) sidecar precheck: `hooks/rate-limit-precheck.sh` hashes incoming cmd, lifts `retry_count` into `RATE_LIMIT_RETRY_COUNT` env, removes queue entry; (flock) `RateLimitQueue` read-modify-write wrapped with `fcntl.flock(LOCK_EX)` 5s timeout, atomic `os.replace()` writes, timeout logged. 16 new tests (3 flock unit + 6 drainer integration + 5 precheck integration + atomic-lock reaper TOCTOU fix). Both hooks registered in `apply-efficiency-profile.sh` standard profile. | hooks/rate-limit-drain.sh, hooks/rate-limit-precheck.sh, lib/rate_limiter.py | **RESOLVED fully** |
