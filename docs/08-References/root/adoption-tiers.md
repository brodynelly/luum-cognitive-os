...
| Hooks wired | ~108 (minimal profile) | ~144 (standard profile) | ~160 (paranoid profile) |
...
| Primary cost | ~108 hook fires/turn | ~144 hook fires/turn | ~160 hook fires/turn + engram I/O |
...
(`_hook_count: 108`). Enable by copying that profile into `.claude/settings.json`
...
`templates/security-profiles/standard.json` (`_hook_count: 144`).
...
`templates/security-profiles/paranoid.json` (`_hook_count: 160`).
...
The OS fires hooks on every tool call. At lean tier that is ~108 fires; even
...
| Hook fires per turn | ~108, 0 daemons | ~144, 1 daemon (Engram) | ~160, Engram hot-path |
...
<!-- Generated from c1f4b385 on 2026-05-15T23:17:12Z. Do not edit directly. Run `python3 scripts/render_adoption_tiers.py` to regenerate. -->
