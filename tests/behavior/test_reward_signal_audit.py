...
def _write_reward_fixture(tmp_path):
...
""".strip(),
    ...
    return metrics_dir, contract


@pytest.mark.behavior
def test_reward_signal_audit_cli_reports_quarantined_rows(project_root, tmp_path):
    _, contract = _write_reward_fixture(tmp_path)
    result = subprocess.run(
        [
            str(project_root / "scripts" / "cos-reward-signal-audit"),
            "--project-dir",
            str(tmp_path),
            "--contract",
            str(contract),
...
def test_reward_signal_audit_cli_repair_dry_run(project_root, tmp_path):
    metrics_dir, contract = _write_reward_fixture(tmp_path)
