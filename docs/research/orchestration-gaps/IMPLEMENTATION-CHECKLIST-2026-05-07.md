...
| ADR-230 | Handoff Envelope + Cycle Deduplication | 🟡 Slices F–G ✅ | File-IPC inbox transport plus explicit receiver execution via `cos team handoff receive --exec-command-template`; cross-harness inbox contract covered. Remaining: external hook runner and kill-mid-dispatch chaos. | T1, T2, T3, T4, T5, T8 done; T7 pending |
...
| ADR-233 | Cross-Session Agent-Team File IPC | 🟡 Slices A–C ✅ | File-backed AgentTeam substrate, `cos team ...`, TaskCreated/TaskCompleted/TeammateIdle consumers, ADR-230 inbox handoff transport, chaos claim race, cross-harness inbox contract. Remaining: receiver execution + NATS/A2A upgrade path. | T1, T3, T4, T7, T8 done |
