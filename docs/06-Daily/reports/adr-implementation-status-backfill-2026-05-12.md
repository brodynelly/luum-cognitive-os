...
| Metric | Count |
|---|---:|
| Total tracked ADR files | 285 |
| Tracked ADRs with YAML frontmatter | 285 |
| Tracked ADRs still prose-only | 0 |
| Active ADRs | 244 |
| Active with implementation_status | 244 |
...
| [007](../adrs/ADR-007-cognitive-os-rebrand.md) | `accepted` | `not-applicable` | governance-only: naming/prose rebrand decision; intentionally no physical rename implementation surface | **Date:** 2026-03-24 |
| [008](../adrs/ADR-008-multi-tool-support.md) | `accepted` | `partial` | architecture policy with broad adapter direction; later harness adapters exist, but this ADR is not itself closed as a complete multi-tool implementation | **Date:** 2026-03-28 |
| [009](../adrs/ADR-009-package-architecture.md) | `accepted` | `implemented` | package taxonomy exists in repository package/manifests layout; ADR is a historical classification decision rather than open runtime work | **Date:** 2026-03-28 |
| [010](../adrs/ADR-010-hook-architecture-v2.md) | `accepted` | `implemented` | security profiles and hook profile scripts exist; ADR consequences describe the profile system as operational | **Date:** 2026-03-28 to 2026-04-13 |
...
| [023](../adrs/ADR-023-updated-input-pattern.md) | `accepted` | `partial` | updatedInput behavior exists in secret-detector, but related advisory/profile surfaces named by the ADR are not fully present | **Date:** 2026-04-15 |
...
| [037](../adrs/ADR-037-self-knowledge-base.md) | `accepted` | `implemented` | self-knowledge generator, SessionStart refresh hook, and query artifacts are present as the ADR implementation surface | Sub-agents spend 3-10K tokens per session grepping source files to answer basic questions: |
...
| [088](../adrs/ADR-088-provenance-trailer-ppid-chain.md) | `accepted` | `implemented` | commit provenance marker/trailer implementation and behavior tests exist for the PPID attribution fix | Accepted. |
...
| [092](../adrs/ADR-092-harness-skills-sync-path.md) | `accepted` | `implemented` | self-install resolves claude skills destinations and syncs skills into harness-specific paths | As of 2026-04-16, the project has 126 skill directories under `skills/`. The Claude Code harness |
...
| [099](../adrs/ADR-099-pre-agent-snapshot-copy-on-untracked.md) | `accepted` | `implemented` | pre-agent snapshot, crash recovery, cleanup, snapshot manager, and integration tests implement copy-on-untracked behavior | **Supersedes**: (part of ADR-003 Mechanism A) |
...
| [103](../adrs/ADR-103-audit-contract-lane-recovery.md) | `accepted` | `not-applicable` | policy-only: lane sequencing decision; no standalone implementation surface beyond later audit/contract lane work | Accepted. |
...
| [112](../adrs/ADR-112-codex-governed-tool-layer.md) | `accepted` | `implemented` | governed runner and Codex guard scripts exist and list canonical hook chains for Codex surfaces | Accepted — 2026-05-02. |
...
| [114](../adrs/ADR-114-hook-quality-system.md) | `accepted` | `implemented` | hook-quality manifest, audit script, and contract tests implement the hook quality system | Accepted — 2026-05-02. |
...
| [120](../adrs/ADR-120-conversation-to-primitive-harvester.md) | `accepted` | `partial` | primitive harvester architecture exists, but this ADR is not evidenced as a fully closed runtime harvester implementation | Accepted — 2026-05-02 |
...
| [122](../adrs/ADR-122-preflight-gate-refinements.md) | `accepted` | `not-applicable` | policy-only: refinement decision for preflight gates; implementation is distributed across later gate ADRs | **Author**: Maintainer (operator) |
...
| [133](../adrs/ADR-133-expansion-without-monsterization.md) | `accepted` | `not-applicable` | governance-only: architectural scope-control doctrine with no direct implementation work item | Accepted — 2026-05-03 |
...
| [172](../adrs/ADR-172-multi-surface-ui-architecture.md) | `accepted` | `not-applicable` | governance-only: architecture/doctrine ADR assigns existing surfaces; implementation evidence is a snapshot rather than a direct work item | Accepted. Supersedes [ADR-170](ADR-170-operator-cli-as-primary-ui-surface.md). |
...
| [178](../adrs/ADR-178-openharness-primitive-adoption.md) | `accepted` | `partial` | OpenHarness adoption is pattern/primitive adoption work; later projections exist but full adoption is not closed here | **Deciders**: Maintainer |
...
| [192](../adrs/ADR-192-surface-5-adopt-bubbletea.md) | `accepted` | `implemented` | Bubble Tea adoption has Go TUI package/compile proof while existing Python bridge remains functional | Accepted — 2026-05-06 |
...
| [198](../adrs/ADR-198-release-external-readiness-gate.md) | `accepted` | `implemented` | release external readiness script and contract tests implement the read-only preflight gate | Accepted — 2026-05-06 |
...
| [200](../adrs/ADR-200-state-retention-controller.md) | `accepted` | `implemented` | state retention manifest, audit hook/script, reaper flow, and tests implement the controller scope | Accepted — 2026-05-06 |
| [201](../adrs/ADR-201-maintainer-agent-telemetry-promotion-loop.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Report**: `docs/06-Daily/reports/self-improvement-maintainer-agent-gap-2026-05-06.md` |
| [202](../adrs/ADR-202-private-content-cross-harness-portability-boundary.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Report**: `docs/06-Daily/reports/private-content-portability-gap-2026-05-06.md` |
| [203](../adrs/ADR-203-subagent-capability-contract-and-launch-preflight.md) | `accepted` | `implemented` | subagent capability manifest, launch preflight script, CLI route, telemetry, and tests implement the contract | **Implementation**: `manifests/subagent-capabilities.yaml`, `scripts/subagent_launch_preflight.py`, `scripts/cos subagent preflight` |
...
| [212](../adrs/ADR-212-cross-stack-license-audit-toolchain.md) | `accepted` | `implemented` | cross-stack license audit manifest, CLI wrapper, scanner scripts, and tests implement the audit toolchain | **Source**: Q2 tool-adoption review, `.cognitive-os/strategy/research/11-cross-stack-license-audit-tools.md` |
| [213](../adrs/ADR-213-agent-preflight-before-stash-snapshot.md) | `accepted` | `implemented` | implementation/shipped/delivered evidence | **Source**: `docs/06-Daily/reports/stash-hidden-wip-postmortem-2026-05-06.md` |
...
| [216](../adrs/ADR-216-tool-discovery-pre-use-gate.md) | `accepted` | `implemented` | tool-discovery pre-use manifest, evaluator CLI, Bash gate integration, and tests implement the gate | **Source**: repeated dogfood evidence of ad-hoc external tool selection over existing COS primitives |
...
| [223](../adrs/ADR-223-agent-lifecycle-reconstruction.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: `docs/03-PoCs/research/multi-agent-orchestration-prior-art-2026-05-06.md`, `docs/03-PoCs/research/orchestration-gaps/background-agent-patterns.md`, and the operator |
| [225](../adrs/ADR-225-branch-per-task-mode.md) | `accepted` | `partial` | Slices A-B are implemented; automatic branch migration for existing worktrees remains explicitly not implemented | Worktree-per-write-agent isolates filesystem mutations, but branch identity still needs a stable operator-visible contract. Without a branch-per-task policy, de |
...
| [227](../adrs/ADR-227-shadow-git-checkpoint-substrate.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: [`docs/03-PoCs/research/orchestration-gaps/replay-timeline-architectures.md`](../research/orchestration-gaps/replay-timeline-architectures.md). Cline, Herme |
...
| [230](../adrs/ADR-230-handoff-envelope-and-cycle-deduplication.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: [`docs/03-PoCs/research/orchestration-gaps/agent-to-agent-handoff.md`](../research/orchestration-gaps/agent-to-agent-handoff.md). Production failure rate of |
| [231](../adrs/ADR-231-mcp-server-surface-for-cos-primitives.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: [`docs/03-PoCs/research/orchestration-gaps/mcp-as-orchestration-bus.md`](../research/orchestration-gaps/mcp-as-orchestration-bus.md) |
...
| [233](../adrs/ADR-233-cross-session-agent-team-file-ipc.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: [`docs/03-PoCs/research/orchestration-gaps/cross-session-agent-teams.md`](../research/orchestration-gaps/cross-session-agent-teams.md) |
| [234](../adrs/ADR-234-approval-policies-as-code.md) | `accepted` | `implemented` | Slices A-C are implemented for the COS-native policy evaluator, migrated hooks, and settings projection plan | COS has many shell hooks with embedded allow/deny logic. Research recommended a COS-native YAML policy evaluator before adopting heavy engines such as OPA, Ceda |
| [235](../adrs/ADR-235-detached-agent-daemon.md) | `accepted` | `implemented` | explicit accepted/implemented status | **Source**: [`docs/03-PoCs/research/orchestration-gaps/background-agent-patterns.md`](../research/orchestration-gaps/background-agent-patterns.md) |
| [236](../adrs/ADR-236-deferred-tool-loading-and-toolsearch.md) | `accepted` | `partial` | Slices A-D are implemented; real MCP list_changed transport emission remains explicitly not implemented | The orchestration research recommended adopting the ToolSearch/deferred-loading pattern instead of loading every tool schema into every session. This is not a s |
...
| [260](../adrs/ADR-260-grant-signed-cosd-api.md) | `accepted` | `implemented` | cosd grant modules, nonce store, daemon integration, and tests implement the grant-signed API scope | **Date:** 2026-05-11 |
| [261](../adrs/ADR-261-memory-governance-v2.md) | `accepted` | `implemented` | memory governance module, retriever/lifecycle integration, and unit tests implement typed memory policy | **Date:** 2026-05-11 |
| [263](../adrs/ADR-263-tool-replay-budget-ledger.md) | `accepted` | `implemented` | tool replay ledger module and tests implement per-session replay budget decisions | **Date:** 2026-05-11 |
| [264](../adrs/ADR-264-tool-result-envelope.md) | `accepted` | `implemented` | tool result envelope module, agent/dispatch integration, spillover behavior, and tests implement compact result envelopes | **Date:** 2026-05-11 |
...
| [268](../adrs/ADR-268-history-sanitization-2026-05-11.md) | `accepted` | `implemented` | history sanitization action is documented as performed and reinforced by ADR-267 forward enforcement | Accepted (2026-05-11) |
...
| [272](../adrs/ADR-272-structural-rule-backend-boundary.md) | `accepted` | `not-applicable` | policy-only: boundary decision constraining future structural backend integrations; no runtime implementation is claimed | Status: Accepted |
...
| [275](../adrs/ADR-275-closure-and-projection-primitives.md) | `accepted` | `partial` | Slice A projector and close primitive are implemented; hook wiring across harnesses remains staged for operator review | Accepted — Slice A implemented (projector + close primitive + tests). Hook |
