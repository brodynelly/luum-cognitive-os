...
| `docs/architecture/plans/` | 5 | NO |
| `docs/plans/` (legacy) | 1 (`SO-RELIABILITY-MEGA-PLAN.md`) | NO |
| `docs/archive/plans/` | 1+ (`self-optimizing-pipeline.md`) | NO |
...
| Range | Count |
|---|---|
| ADR-027 → ADR-081 (present in `docs/adrs/`) | 58 files (including sub-ADRs: 027a, 028a/b/c, 029b, 033b, 055b) |
| ADRs referenced by plans but NOT in `docs/adrs/` | 2 (ADR-002, ADR-012) |

---

## B. Confirmed Duplications

### 1. ADR-081 ↔ `docs/architecture/plans/adr-064-implementation-plan.md` (the triggering case)
...
### 3. `docs/architecture/plans/adr-064-implementation-plan.md` ↔ ADR-064
...
### 4. `docs/architecture/plans/runtime-comparison-benchmark-plan.md` and `headless-clustered-runtime-plan.md` — both orphan-reference ADR-027
...
### D1. Plans referencing ADRs that do not exist in `docs/adrs/`
...
ADR-002 and ADR-012 are referenced but have no corresponding file in `docs/adrs/`. These are likely pre-numbering-convention decisions that were never formalized as files, or the files were deleted. The plans that reference them cannot be validated.
...
| ADR-028a | `.cognitive-os/plans/features/self-optimizing-pipeline.md` | NO — file is at `docs/archive/plans/self-optimizing-pipeline.md` (archived) |
...
|---|---|---|
| `adr-064-implementation-plan.md` | `docs/architecture/plans/` | `.cognitive-os/plans/features/` (operational plan, not architecture doc) |
| `governed-self-improvement-roadmap.md` | `docs/architecture/plans/` | Could be either; no ADR, so arguably premature for `docs/architecture/plans/` |
| `SO-RELIABILITY-MEGA-PLAN.md` | `docs/plans/` (third location) | `docs/archive/plans/` (its content is superseded by ADR-028) |
| `self-optimizing-pipeline.md` | `docs/archive/plans/` | Correctly archived, but ADR-028a still refers to it as `.cognitive-os/plans/features/` |
...
- `session-startup-protocol.sh` scans only `.cognitive-os/plans/features/` — `research/` and all `docs/architecture/plans/` are invisible to the startup summary.
...
| 2 | `docs/architecture/plans/adr-064-implementation-plan.md` ↔ ADR-081 | **HIGH** | Triggered this audit; plan is NOT in the indexed directory; ADR-081 reconciled it manually but only by chance |
