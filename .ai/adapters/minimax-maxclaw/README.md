# MiniMax MaxClaw / MiniMax Agent portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `minimax-maxclaw`
- status: `planned`
- proof level: `none`
- projection mode: `hosted-agent-or-openclaw-compatible`

## Settings paths

- none declared

## Rule

Read `.ai/profiles/minimax-maxclaw.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
