...
- [x] **AC3**: `cos-test focused` completes in <30s for a typical 1–3 file diff (measured on changes touching `scripts/decision_triage.py`; legacy plan text named nonexistent `lib/decision_triage.py`).
  - Verified 2026-05-20: after focused-mode fast-lane mapping and decision-triage Engram timeout bounding, `/tmp/cos-test focused --ci --no-testmon --changed-files scripts/decision_triage.py` selected `tests/unit/test_decision_triage.py` only and passed in 14.79s wall time (`21 passed in 11.61s`). Earlier 89–126s runs proved the stale behavior selected slow integration tests and waited on Engram subprocess timeouts.
