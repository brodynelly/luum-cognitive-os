...
- `install.sh --harness` now exposes the implemented `cos_init.py` harness set beyond Claude/Codex for first-run structural projections, with a Cursor smoke in `tests/integration/test_installer.py`; remaining work is to remove the duplicated bash harness list and make shell settings-driver helpers preserve structural harness metadata instead of falling back to Claude. See [Install Harness UX Gap Review — 2026-05-15](install-harness-ux-gap-2026-05-15.md).
