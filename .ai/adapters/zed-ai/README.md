# Zed AI portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `zed-ai`
- status: `implemented`
- proof level: `structural`
- projection mode: `zed-project-rules-context-servers`

## Settings paths

- `.rules`
- `.zed/settings.json`

## Rule

Read `.ai/profiles/zed-ai.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
