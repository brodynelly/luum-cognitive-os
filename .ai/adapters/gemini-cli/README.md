# Gemini CLI portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `gemini-cli`
- status: `implemented`
- proof level: `structural`
- projection mode: `gemini-project-settings-context-mcp`

## Settings paths

- `.gemini/settings.json`
- `GEMINI.md`

## Rule

Read `.ai/profiles/gemini-cli.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
