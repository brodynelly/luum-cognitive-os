...
|---|---|---|
| Self-building claim | Signed for the narrow claim "self-instrumenting under operator approval". | `scripts/cos_primitive_harvester.py` produced `docs/reports/primitive-harvester-promotion-evidence-2026-05-04.json`; `scripts/cos_primitive_harvester` moved from `sandbox` to `advisory` with `promotion_evidence.primary_signal=primitive-harvester` and `approved_by=operator`. |
...
| ADR ≤138 closure | Classified, not blindly implemented. | `docs/SESSION-ADR-CLOSURE-2026-05-04.md` assigns all 50 attention ADRs to evidence-only, absorbed, superseded, obsolete-by-context, or deferred; `implement-current=0`. |
