# Amp Code portable `.ai` adapter

Schema: `portable-ai-adapter.v1`

This adapter is generated from Cognitive OS canonical primitive manifests.
It must not invent primitive behavior or overclaim runtime enforcement.

## Current projection

- harness id: `amp-code`
- status: `implemented`
- proof level: `structural`
- projection mode: `agents-md-workspace-settings-mcp`

## Settings paths

- `AGENTS.md`
- `.amp/settings.json`

## Rule

Read `.ai/profiles/amp-code.json` for declared fidelity before projecting primitives into this host.
Structural advisory surfaces are not runtime enforcement.
